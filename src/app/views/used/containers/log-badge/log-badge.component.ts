...
@Component({
  ...
  styleUrls: ['./log-badge.component.css']
})
export class LogBadgeComponent implements OnInit {
  ...
  async ngOnInit() {
    if ((window as any).ethereum) {
      ...
      this.badge = badge.updates;

      for (let index = 0; index < badge.updates.length; index++) {
        const element = badge.updates[index];
        console.log('{element}', element, allBadges[element]);
      }
      // const badgesSpace = await box.openSpace(`${accounts[0]}_badges`);
    } else {
